...
```{r}
library(readxl)
param <- read_excel("param_new recovery rate_DSA.xlsx")
range <- read_excel("param_all_owsa.xlsx")
...
df_psa <- function(params) {
  # run following code with a set of data 
    ...
    v_totaldisc_tx_RT=numeric(),
      v_totaldisc_tx_PT=numeric(),
      productivity_loss_perpatient=numeric(),
      c_travel_RT=numeric(),
      c_travel_PT=numeric(),
      v_tendc_RT=numeric(),
      v_tendc_PT=numeric(),
...
result_list <- list()
  ...
  for(i in 1:nrow(df_psa_input)){ 
    ...
    a_P_PT_total <- array(0,  # Create 3-D array
                dim = c(n_states, n_states, n_cycles),
                dimnames = list(v_names_states, v_names_states, 
                                v_names_cycles[-length(v_names_cycles)]))
...
p_mort_PT<-p_mort_new*df_psa_input[i,"HR"]
p_PFS_PT<-1-(p_mort_PT*coeff_PFS)

a_P_PT_total["PF", "PF",] <- 1-(p_mort_PT*coeff_PFS)
a_P_PT_total["PF", "PD",] <- (1-p_mort_new)-p_PFS_PT
a_P_PT_total["PF", "D",]  <- (p_mort_new)

# from PD
a_P_PT_total["PD", "PF",] <- 0
a_P_PT_total["PD", "PD",] <- 1-(p_mort_PT)
a_P_PT_total["PD", "D",]  <- p_mort_PT
...
i_dis_QALY<- v_dis_totalu_PT - v_dis_totalu_RT 
 ...
 results_df <- rbind(results_df, data.frame(
      ...
      ICER_dis= ICER_dis,
      ICER = ICER,
      i_dis_QALY= i_dis_QALY# Make a dataframe from the matrix 
     ))
...
#DSA
 ...
 #write.xlsx(result_table, "DSA_result_cohort.xlsx")
...
library(openxlsx)
write.xlsx(df_tornado, "df_tornado_all_owsa_cohort_20.xlsx")
```

```{r}
new_column_names <- c("cost_DY_RT","cost_DY_PT","Cost_tx_RT","Cost_tx_PT", "productivity_loss"
     ,"Cost_travel_RT"
      ,"Cost_travel_PT",
      "Cost_end_RT", "Cost_end_PT","Cost_future_RT", "Cost_future_PT"
,"Costs_PT", "Costs_RT", "LY_RT","LY_PT", "QALY_PT", "QALY_RT","ICER")
colnames(result_table) <- new_column_names

new_row_names <- paste("Iteration", 1:100, sep = " ")
rownames(result_table) <- new_row_names

result_table<- result_table %>% mutate(
  inc_QALY=QALY_PT-QALY_RT,
  inc_LY = LY_PT-LY_RT,
  inc_Cost = Costs_PT- Costs_RT
)
RT<-list( result_table[, "LY_RT"],result_table[, "QALY_RT"],result_table[, "Costs_RT"]
  
)
PT<-list( result_table[, "LY_PT"],result_table[, "QALY_PT"],result_table[, "Costs_PT"]
...
df_output <- as.data.frame(m_output)
 ...
 l_v_IncrQALY_PSA  <- result_table[, "QALY_PT"]  - result_table[, "QALY_RT"] 
 l_v_IncrCosts_PSA <- result_table[, "Costs_PT"] - result_table[, "Costs_RT"]

 
 df_output$iLY    <- result_table[, "LY_PT"]    - result_table[, "LY_RT"] 
 df_output$iQALY  <- result_table[, "QALY_PT"]  - result_table[, "QALY_RT"] 
 df_output$iCosts <- result_table[, "Costs_PT"] - result_table[, "Costs_RT"]
...
l_df_PSA_QALY <- df_PSA_QALY <- data.frame(Strategy = v_names_trt,
                     Cost = c(mean(m_output[, "Costs_RT"]), mean(m_output[, "Costs_PT"])),
                     QALY = c(mean(m_output[, "QALY_RT"]),  mean(m_output[, "QALY_PT"])),
                     Inc_Cost = c(NA, mean(l_v_IncrCosts_PSA)),
...
plot_inc_CE_plane_QALY<-plot(plot_inc_CE_plane_QALY)+
  labs(title     = paste("Incremental Cost-effectiveness plane")
         ) +
    ...
    geom_vline(xintercept = 0, color = "gray", size = 0.6) +
  geom_hline(yintercept = 0, color ="gray", size = 0.6) +
  geom_abline(intercept = 0, slope = 50000, color = "red", linetype = "solid")+
  geom_abline(intercept = 0, slope = 80000, color = "red", linetype = "longdash")+
        scale_color_manual(values = c("blue", "black")) + 
    scale_fill_manual(values = c("blue", "black"))


#ggsave("plot_inc_CE_plane_QALY.pdf", plot = plot_inc_CE_plane_QALY, width = 8, height = 6, units = "in", device = "pdf")
