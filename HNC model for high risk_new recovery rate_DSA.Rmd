...
n_year    <- 50
n_t       <- n_year*6                         # number of cycle * years (50 years)
...
cdr_2m <- (1+cdr) ^ (1/6) - 1 
odr_2m <- (1+odr) ^ (1/6) - 1 
v_dwc <- 1 / ((1 + cdr_2m) ^ (0:(n_cycles)))       # vector with discount weights for costs
v_dwe <- 1 / ((1 + odr_2m) ^ (0:(n_cycles)))      # vector with discount weights for outcomes
set.seed(123)  # set the seed
n_trt<-2
v_names_trt<-c("RT","PT")
n_iter<-100
 
n_year_FE<- ifelse(n_year > 35, 35,n_year)
```

```{r}
library(readxl)
param <- read_excel("param2_new recovery rate_DSA.xlsx")
range <- read_excel("param2_all_owsa.xlsx")
...
p_mort_compare <- p_mort_compare %>% mutate(
  max_mort <- pmax(m_rate, p_mort_compare)
)
p_mort_new <- p_mort_compare$max_mort 
p_mort_new<-as.matrix(p_mort_new)
p_mort_new <- head(p_mort_new, n_cycles)
coeff_PFS<-1.53
...
c_XE_RT <-
          c_XE2*(1-df_psa_input[i,"ratio_XE3_RT"])+
          c_XE3*(df_psa_input[i,"ratio_XE3_RT"])

c_XE_PT <-
          c_XE2*(1-df_psa_input[i,"ratio_XE3_PT"])+
          c_XE3*(df_psa_input[i,"ratio_XE3_PT"])



c_DY2<-
      ((df_psa_input[i,"ru_nutrition_DY2"]*df_psa_input[i,"p_ru_nutrition_DY2"]*df_psa_input[i,"uc_nutrition"])+
...
c_DY_RT <-
          c_DY2*(1-df_psa_input[i,"ratio_DY3_RT"])+
          c_DY3*(df_psa_input[i,"ratio_DY3_RT"])

c_DY_PT <-
          c_DY2*(1-df_psa_input[i,"ratio_DY3_PT"])+
...
c_MU_RT <-
          c_MU2*(1-df_psa_input[i,"ratio_MU3_RT"])+
          c_MU3*(df_psa_input[i,"ratio_MU3_RT"])

c_MU_PT <-
          c_MU2*(1-df_psa_input[i,"ratio_MU3_PT"])+
...
#u_XE
u_XE_RT <-
          df_psa_input[i,"u_XE2"]*(1-df_psa_input[i,"ratio_XE3_RT"])+
          df_psa_input[i,"u_XE3"]*(df_psa_input[i,"ratio_XE3_RT"])

u_XE_PT <-
          df_psa_input[i,"u_XE2"]*(1-df_psa_input[i,"ratio_XE3_PT"])+
          df_psa_input[i,"u_XE3"]*(df_psa_input[i,"ratio_XE3_PT"])

#u_DY
u_DY_RT <-
          df_psa_input[i,"u_DY2"]*(1-df_psa_input[i,"ratio_DY3_RT"])+
          df_psa_input[i,"u_DY3"]*(df_psa_input[i,"ratio_DY3_RT"])

u_DY_PT <-
          df_psa_input[i,"u_DY2"]*(1-df_psa_input[i,"ratio_DY3_PT"])+
          df_psa_input[i,"u_DY3"]*(df_psa_input[i,"ratio_DY3_PT"])


#u_MU
u_MU_RT <-
          df_psa_input[i,"u_MU2"]*(1-df_psa_input[i,"ratio_MU3_RT"])+
          df_psa_input[i,"u_MU3"]*(df_psa_input[i,"ratio_MU3_RT"])

u_MU_PT <-
          df_psa_input[i,"u_MU2"]*(1-df_psa_input[i,"ratio_MU3_PT"])+
...
u_PF<-c(rep(df_psa_input[i,"u_PF"],60), rep(u_PF_older,n_cycles-59))
u_PD<-c(rep(df_psa_input[i,"u_PD"],60), rep(u_PD_older,n_cycles-59))
u_D <-c(rep( df_psa_input[i,"u_D"],n_cycles+1))
...
v_diffc_tx<-v_totalc_tx_PT-v_totalc_tx_RT
...
for(j in 1:n_year_FE){
  
    a_c_endlife[((6*j)-5):(j*6), "new_dead_n"] <- df_psa_input[i,paste0("c_endlife_", j)]
}
if (n_year >= 35){
a_c_endlife[((6*n_year_FE)-5):((6*n_year_FE)+1), "new_dead_n"] <- df_psa_input[i,"c_endlife_35"]
}
...
if (n_year >= 35){
a_c_futurecost[((6*n_year_FE)-5):((6*n_year_FE)+1),"PF" ] <- df_psa_input[i,"c_futurecost_35"]
a_c_futurecost[((6*n_year_FE)-5):((6*n_year_FE)+1),"PD" ] <- df_psa_input[i,"c_futurecost_35"]
}
...
v_diffu <-v_totalu_PT-v_totalu_RT
...
i_dis_QALY<- v_dis_totalu_PT - v_dis_totalu_RT 
  ...
  results_df <- rbind(results_df, data.frame(
      ...
      productivity_loss_perpatient=productivity_loss_perpatient,
      c_travel_RT=c_travel_RT,
      c_travel_PT=c_travel_PT,
      v_tendc_RT=v_tendc_RT,
      v_tendc_PT=v_tendc_PT,
      v_tdisfc_RT=v_tdisfc_RT,
      v_tdisfc_PT=v_tdisfc_PT,
      v_tfc_RT=v_tfc_RT,
      v_tfc_PT=v_tfc_PT,
      v_tc_PT = v_tc_PT,
      v_tc_RT = v_tc_RT,
      ...
      ICER_dis= ICER_dis,
      ICER = ICER,
      i_dis_QALY= i_dis_QALY# Make a dataframe from the matrix  # Make a dataframe from the matrix 
     ))
...
#DSA
 ...
 param_psa <- make_psa_df(param = param, n_iter = 1, seed = 123)

 df_psa_input <- gen_psa(param_psa)
 
 psa_result<-df_psa(df_psa_input)
 
 result_table <-psa_result[[1]]

```

```{r}
beta_params(0.308964, 0.1*0.308964)
gamma_params(154.4, 0.2*154.4)
```
...
```{r}
#OWSA
 param_psa <- make_psa_df(param = param, n_iter = 2, seed = 123)

 df_psa_input <- gen_psa(param_psa)
 

replace_with_min_max_row <- function(df_psa_input, range) {
   new_dfs <- list()
  
  for (i in 1:nrow(range)) {
    
    param_names <- range[i, "pars"]
    min_val <- range[i, "min"]
    max_val <- range[i, "max"]
    
      new_df <- df_psa_input
    # Find column index for parameter
    col_index <- match(param_names, names(new_df))
    
    # Replace value in first row with min_val
    new_df[1, col_index] <- min_val
    
    # Replace value in second row with max_val
    new_df[2, col_index] <- max_val
    
    new_name <-i
   
    new_dfs[[i]] <- new_df
    
    names(new_dfs)[i] <- new_name
  }
...
updated_data <- replace_with_min_max_row(df_psa_input, range)
...
results <- list()
for (i in 1:168){
  df_psa_input<-updated_data[[i]]
  psa_result<-df_psa(df_psa_input)

  result_table <-psa_result[[2]]
  
 results[[i]] <-psa_result
}
...
```{r}
icer_parts <- lapply(results, function(x) x[[2]][["ICER"]])
txcost_parts <- lapply(results, function(x) x[[2]][["v_diffc_tx"]])
utility_parts <- lapply(results, function(x) x[[2]][["v_diffu"]])

# Combine all extracted "ICER" data into a single dataframe
df_tornado <- do.call(rbind, icer_parts)
df_tornado_txcost <- do.call(rbind, txcost_parts)
df_tornado_u <- do.call(rbind, utility_parts)

# The df_tornado now contains the combined rows from all "ICER" elements in results


df_tornado<-cbind(range[,"pars"],df_tornado)
df_tornado_u<-cbind(range[,"pars"],df_tornado_u)
df_tornado_txcost<-cbind(range[,"pars"],df_tornado_txcost)
names(df_tornado)<-c("Parameter","min","max")
names(df_tornado_u)<-c("Parameter","min","max")
names(df_tornado_txcost)<-c("Parameter","min","max")
df_tornado$diff=df_tornado$max-df_tornado$min
names(df_tornado)<-c("Parameter","min","max","diff")
library(openxlsx)
write.xlsx(df_tornado, "df_tornado_all_owsa_high_risk_20.xlsx")
```
...
```{r}
 plot_inc_CE_plane_LY <- make_psa_obj(cost  = as.data.frame(l_df_output_incr$iCosts), 
                               effectiveness = as.data.frame(l_df_output_incr$iLY), 
...
plot(plot_inc_CE_plane_LY)+
 ...
 plot_inc_CE_plane_QALY <- make_psa_obj(cost  = as.data.frame(l_df_output_incr$iCosts), 
                               effectiveness = as.data.frame(l_df_output_incr$iQALY), 
                               strategies    = "Incremental")

plot_inc_CE_plane_QALY_highrisk<-plot(plot_inc_CE_plane_QALY)+
  labs(title     = paste("Incremental Cost-effectiveness plane")
         ) +
...
ggsave("plot_inc_CE_plane_QALY_highrisk.png", plot = plot_inc_CE_plane_QALY_highrisk, width = 8, height = 6, units = "in", device = "png")
...
CEAC_highrisk<- ggplot(ceac_data_long, aes(x = WTP/1000, y = Probability, color = Outcome)) +
  geom_line() +
...
ggsave("CEAC_highrisk.png",CEAC_highrisk, width = 10, height = 8, dpi = 300)
